import java.util.Scanner;

public class SM4 {

    int[] MK;
    public SM4(int[] M){
        this.MK=M;
    }


    //密钥扩展常数
    final int[] FK={
            0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
    };
    //固定参数
    final private int[] CK = {
            0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
            0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
            0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
            0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
            0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
            0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
            0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
            0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
    };
    //S盒
    final private int[] S = {
            
    };

    //循环左移，place是左移的位置
    private int ROL(int X, int place) {
        return ((X << place) | (X >> (32 - place)));
    }


    public byte S_box(byte a){
        byte b;
        b=(byte)S[a];
        return b;
    }
    //非线性变换tao
    private int tao(int a){
        byte[] b=new byte[4];
        b[0]=(byte)S[(a>>24)&0xff];
        b[1]=(byte)S[(a>>16)&0xff];
        b[2]=(byte)S[(a>>8)&0xff];
        b[3]=(byte)S[a&0xff];
        int temp;
        temp=((int)(b[0]<<24)|(int)(b[1]<<16)|(int)(b[2]<<8)|(int)(b[3]));
        return temp;
    }

    //线性变换部件L
    private int L(int B){
        int C;
        C=B^(ROL(B,2))^(ROL(B,10))^(ROL(B,18))^(ROL(B,24));
        return C;

    }

    //线性变换部件L1
    private int L1(int B){
        int C;
        C=B^(ROL(B,13))^(ROL(B,23));
        return C;
    }

    //合成变换T,L和tao的复合
    private int T(int X){
        int temp;
        temp=L(tao(X));
        return temp;
    }

    //合成变换T1,L1和tao的复合
    private int T1(int X){
        int temp;
        temp=L1(tao(X));
        return temp;
    }

    //轮函数1
    private int F(int[] X, int rk){
        return X[0]^T(X[1]^X[2]^X[3]^rk);
    }

    //加密算法或者解密算法,加密传明文，解密传密文，密钥不变
    public int[] Encryption(int[] X){


        int[] rk=keyExtension();//扩展密钥
        int[] Y=new int[4];
        int X4;

        for(int i=0;i<32;i++){
            X4=F(X,rk[i]);
            for(int j=0;j<3;j++){
                X[j]=X[j+1];
            }
            X[3]=X4;
        }

        System.out.println();
        Y[0]=X[3];
        Y[1]=X[2];
        Y[2]=X[1];
        Y[3]=X[0];

        return Y;

    }

    public int[] Decode(int[] X){
        int[] rk=keyExtension();//扩展密钥

        int X4;

        for(int i=31;i>=0;i--){
            X4=F(X,rk[i]);
            for(int j=0;j<3;j++){
                X[j]=X[j+1];
            }
            X[3]=X4;
        }

        int[] Y=new int[4];
        Y[0]=X[3];
        Y[1]=X[2];
        Y[2]=X[1];
        Y[3]=X[0];

       return Y;
    }

    //密钥扩展算法
    public int[] keyExtension(){
        int[] rk=new int[32];
        int[] K=new int[4];//中间数据
        for(int i=0;i<4;i++){
            K[i]=MK[i]^FK[i];//初始化中间数据
        }

        int K4;
        for(int i=0;i<32;i++){
            K4=K[0]^T1(K[1]^K[2]^K[3]^CK[i]);
            rk[i]=K4;
            for(int j=0;j<3;j++){//数组左移
                K[j]=K[j+1];
            }
            K[3]=K4;
        }

        return rk;
    }
}

class SM4demo {
    public static void main(String[] args){
        Scanner reader=new Scanner(System.in);

        String M;
        String C;
        int[] X=new int[4];
        String[] str=new String[4];
        System.out.print("请输入加密的明文:");
        M=reader.next();
        for(int i=0;i<4;i++){
            str[i]=M.substring(i*(M.length()/4),i*(M.length()/4)+M.length()/4);
            X[i]=Integer.valueOf(str[i]);
        }
        //初始化加密秘钥
        int[] MK={1,2,3,4};
        SM4 sm4=new SM4(MK);

        int[] Y;
        Y= sm4.Encryption(X);
        C=Integer.toString(Y[0])+Integer.toString(Y[1])+Integer.toString(Y[2])+Integer.toString(Y[3]);
        System.out.print("密文为："+C+"\n");

//        for(int i=0;i<4;i++){
//            System.out.print(Y[i]+" ");
//        }
       // System.out.println();


        int[] DE=sm4.Decode(Y);
        String s=Integer.toString(DE[0])+Integer.toString(DE[1])+Integer.toString(DE[2])+Integer.toString(DE[3]);
        System.out.print("解密后明文为："+s);
//        for(int i=0;i<4;i++){
//
//            System.out.print(DE[i]+" ");
//        }

    }
}

